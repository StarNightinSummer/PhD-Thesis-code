...
while (i <= replicates)  {
  ...
  if(parEst1$convergence==0){
    ...
    parEst2 = Localfit(data=data, par=parEst1$pars, par1=parEst1$pars, type=type, include.AR=include.AR)  # local QMLE
    ...
    while (parEst2$error==3&j<=4) {
      initial  = parEst2$fit
      parEst2  = Localfit(data=data, par=initial, par1=parEst1$pars, type=type, include.AR=include.AR)   # local QMLE
      print(as.vector(parEst2$fit))
      j=j+1
    }
